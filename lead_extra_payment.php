...
if($_POST['particular'])
{
	if($_POST['particular']=="Re-Launching Application")
	{
		...
		$odr = $obj->insert('dm_3party_payment',$data);

{
		...
		$obj->insert('dm_3party_payment_det',$data3);

}

	header("location:lead_extra_invoice.php?lead=".$_POST['lead'].'&receipt='.$odr);


}

if(isset($_GET['receipt'])!="" && isset($_GET['lead'])!="")
{

	$obj->delete('dm_3party_payment','id='.$_GET['receipt']);

header("location:lead_extra_payment.php?lead=".$_GET['lead']);
...
<table class="table table-striped table-bordered" id="dataTables-Table" style="width:100%">
			  ...
			  <?php 
			  			$i = 1;
			  $rece=$obj->display('dm_3party_payment','leadId='.$_GET['lead']);
			  ...
			  <tr>
								...
								<td>
					 <a href="lead_extra_payment.php?lead=<?=$_GET['lead'];?>&receipt=<?=$rece1['id']?>" class="btnDeleteAction"><i class="fa fa-trash" title="CANCEL BILL"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					 <a href="lead_extra_invoice.php?lead=<?=$_GET['lead'];?>&receipt=<?=$rece1['id']?>" target="_blank" class="btnDeleteAction"><i class="fa fa-print" title="PRINT"></i></a> 
