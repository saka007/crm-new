...
$opsId=$obj->insert('dm_ops_student_visa',$data);
}
else
	$opsId=$_POST['opsId'];
{
...
$obj->update('dm_ops_student_visa',$data,'id='.$opsId);
}
if($_POST['Remark1']!="")
{
	$data2=array(
		'remark1'=> $_POST['Remark1']);
	$obj->update('dm_ops_student_visa',$data2,'id='.$opsId);
}
if($_POST['Remark2']!="")
{
	$data3=array(
		'remark2'=> $_POST['Remark2']);
	$obj->update('dm_ops_student_visa',$data3,'id='.$opsId);
}

if($_FILES['studentFile']['name']!="")
	{
		$filename=time().'_'.$_FILES['studentFile']['name'];
        move_uploaded_file($_FILES['studentFile']['tmp_name'], 'uploads/documents/' . $filename);
			$dataSheet2 = array(
	    			'opsId'   =>  $opsId,
	    			'leadId'  =>  $_POST['lead'],
	    			'tab'   =>  2,
		...
		if($_POST['conversation']!="")
{
	...
	header("location:student_visa_ops.php?lead=".$_POST['lead']);
...
unlink('uploads/documents/'.$d1['file']);
	...
	header("location:student_visa_ops.php?lead=".$_GET['lead']);
}

$ops=$obj->display('dm_ops_student_visa','leadId='.$_GET['lead']);
...
<div class="col-sm-10">
			<h3 class="mb-3">Final Draft for Student Visa</h3> 
...
?>	
  ...
  <div class="row" style="overflow:hidden">
	...
	<div class="col-sm-4 form-group"><label>DOCUMENTS CHECKLIST SENT</label><input type="text" class="form-control" id="date1" name="docSubDate" value="<?php echo($ops1['docSubDate'])?>" required></div>
	<div class="col-sm-4 form-group"><label>DESTINATION/ COUNTRY</label><input type="text" class="form-control" id="" name="descountry" value="<?php echo($ops1['descountry'])?>" required></div>
	<div class="col-sm-4 form-group"><label>UNIVERSITY/COLLEGE NAME</label><input type="text" class="form-control" id="" name="Qualification" value="<?php echo($ops1['university'])?>" required></div>
	<div class="col-sm-12 form-group">
							<?php
							echo "<p>".$ops1['remark1'].'-'.date('d/m/Y').'<p>';
							?>
</div>
	<div class="col-sm-12 form-group"><label>Remark</label><textarea type="text" class="form-control" id="Remark" name="Remark1"></textarea></div>
	<div class="col-sm-3 form-group"><label>Document File </label>
  <input type="file" class="form-control" id="ecaFile" name="studentFile" >
  </div>
   </div>   
   <?php
   if($ops->num_rows > 0)
  ...
  <div class="row" style="overflow:hidden">
	...
	<div class="col-sm-3"><a href="student_visa_ops.php?docId=<?php echo $doc1['id'];?>&lead=<?php echo $_GET['lead'];?>">Delete</a></div>
  </div>
  <?php } }?>
  </div>
  ...
  <div class="col-sm-12 form-group">
						  ...
						  <tbody id="eduBody_b">
						  	<tr>
							    <td><input type="text" required class="form-control" id="date2" name="docReceDate" value="<?php echo($ops1['docReceDate'])?>" ></td>
							    <td><select class="form-control" name="appSub" required>
		<option value="">Select</option>
