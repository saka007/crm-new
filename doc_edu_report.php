...
?>
<div class="col-sm-10">
	<h4 class="mb-3" style="color:#2cb674;">Education Documents report</h4>
	<!-- <div class="col-sm-10">
				...
				<thead>
					...
					<tr>
						...
						<tbody>
							<?php
							if($_SESSION['ID']==7){
							$result = $obj->display3("SELECT *,datediff(CURRENT_DATE,t1.date) as days FROM dm_client_edu as t1 INNER JOIN dm_lead as t2 on t1.leadid=t2.id WHERE datediff(CURRENT_DATE,t1.date)>=0 AND t1.dipthi=1");
							}
							else if($_SESSION['TYPE']=="SA" || $_SESSION['TYPE']=="RMO"){
							    $result = $obj->display3("SELECT *,datediff(CURRENT_DATE,t1.date) as days FROM dm_client_edu as t1 INNER JOIN dm_lead as t2 on t1.leadid=t2.id WHERE datediff(CURRENT_DATE,t1.date)>=0 AND t1.dipthi=1");
							}
							else
							{
							    $result = $obj->display3("SELECT *,datediff(CURRENT_DATE,t1.date) as days FROM dm_client_edu as t1 INNER JOIN dm_lead as t2 on t1.leadid=t2.id WHERE datediff(CURRENT_DATE,t1.date)>=0 AND t1.dipthi=1 and t2.assignTo=".$_SESSION['ID']);
							}
							// print_r($result);die;
